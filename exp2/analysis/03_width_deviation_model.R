...
model <- lmer(width_deviation ~ rm_type * spacing_category * correct_num * correct_width + (1 + rm_type | subID),
...
write.csv(as.data.frame(summary(emm_width_setsize)),
          'exp2/outputs/tables/width_deviation_emmeans_by_width_setsize.csv',
          row.names = FALSE)
...
write.csv(as.data.frame(contrasts_with_d),
          'exp2/outputs/tables/width_deviation_rm_contrasts.csv',
          row.names = FALSE)
