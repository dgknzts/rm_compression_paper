...
contrast_width <- read.csv("exp1/outputs/tables/width_deviation_rm_contrasts.csv")

one_bar_data <- read.csv("exp1/data/one_bar_exp1.csv")
ob_part <- one_bar_data %>%
...
fig_b <- ggplot(w_plot, aes(x = correct_num, y = grand_mean, color = rm_type)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50", linewidth = 0.7) +
  geom_errorbar(data = ob_plot, aes(x = correct_num, y = grand_mean,
                                     ymin = ci_lower, ymax = ci_upper, color = rm_type),
                width = 0.1, linewidth = 0.5, inherit.aes = FALSE) +
  geom_point(data = ob_plot, aes(x = correct_num, y = grand_mean, color = rm_type),
...
ttest_results <- read.csv("exp1/outputs/tables/ttest_results.csv")
...
y_span <- diff(y_range)
y_b1 <- y_range[2] + y_span * 0.20
y_b2 <- y_b1 + y_span * 0.25
y_tick <- y_span * 0.03
...
prec_tests <- read.csv("exp1/outputs/tables/precision_tests.csv")
...
joint_e <- read.csv("exp1/outputs/tables/joint_width.csv") %>%
...
fig_e <- ggplot() +
  ...
  coord_fixed(xlim = c(0, w_max_e * 1.05), ylim = c(-dev_lim_e, 0.2)) +
...
params <- readRDS("exp1/outputs/tables/param_estimates.rds")
...
ggsave("exp1/outputs/figures/comprehensive_results.png", fig_combined,
       width = 12, height = 14, dpi = 300, bg = "white")

ggsave("exp1/outputs/figures/panel_a_number_deviation.png", fig_a, width = 5, height = 5, dpi = 300, bg = "white")
ggsave("exp1/outputs/figures/panel_b_width_deviation.png", fig_b, width = 8, height = 5, dpi = 300, bg = "white")
ggsave("exp1/outputs/figures/panel_c_relative_width.png", fig_c, width = 6, height = 5, dpi = 300, bg = "white")
ggsave("exp1/outputs/figures/panel_d_precision.png", fig_d, width = 5, height = 5, dpi = 300, bg = "white")
ggsave("exp1/outputs/figures/panel_e_iso_rmse.png", fig_e, width = 5, height = 5, dpi = 300, bg = "white")
ggsave("exp1/outputs/figures/panel_f_delta_aic.png", fig_f, width = 6, height = 5, dpi = 300, bg = "white")
ggsave("exp1/outputs/figures/panel_g_compression.png", fig_g, width = 6, height = 5, dpi = 300, bg = "white")
ggsave("exp1/outputs/figures/panel_h_pooling.png", fig_h, width = 4, height = 5, dpi = 300, bg = "white")
